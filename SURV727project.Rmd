...
reinst_tweets_plot2
```
The below plots show a comparison of the top keywords of both corpuses of tweets after each textual cleaning session. After inspection of the second plot for each corpus, we deemed the text of the tweets sufficient for sentiment analysis.
```{r, include=FALSE}
library(patchwork)
...
```
The second sentiment dictionary used in the analysis was the NRC Sentiment dictionary. The barplots below display the count of text that exhibit each of the eight emotions within the Abortion Law Overturned and Abortion Law Reinstated corpuses.
```{r, include=TRUE}
library(syuzhet)
...
Given that the number of tweets vary between both corpuses, we investigated differences in the described emotions as a proportion of the total number of meaningful words in the corpus. The barplots below show the NRC emotion calculations based on a proportionate scale and ordered from highest frequency to lowest frequency.
```{r, include=TRUE}
par(mfrow=c(1,2))
barplot(
